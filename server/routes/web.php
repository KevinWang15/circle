...
//        });
        ...
        $api->group(['prefix' => 'doc'], function ($api) {
            ...
            $api->post('create', 'App\Api\Controllers\DocController@create');
        });
        $api->group(['prefix' => 'misc'], function ($api) {
            $api->post('upload-token', 'App\Api\Controllers\MiscController@uploadToken');
        });
        $api->group(['prefix' => 'qr'], function ($api) {
            $api->post('get-token', 'App\Api\Controllers\QrController@getToken');
            $api->post('set-action', 'App\Api\Controllers\QrController@setAction');
            $api->post('get-response', 'App\Api\Controllers\QrController@getResponse');
            $api->post('set-response', 'App\Api\Controllers\QrController@setResponse');
            $api->post('query', 'App\Api\Controllers\QrController@query');
        });
    });