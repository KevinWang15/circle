...
export class Api {
  ...
  @RequiresLogin
  submitQuiz(result): Promise<quizResultResponse> {
    return this.request("user.submit-quiz", result, {});
  }
  ...
  @RequiresLogin
  newTicket(title: string, category: number, content: string = "", raw_data: string = ""): Promise<Array<any>> {
    return this.request("user.new-ticket", {
      title,
      category,
      content,
      raw_data,
      contact: window['UserService'].contacts
    }, {});
  }
}
